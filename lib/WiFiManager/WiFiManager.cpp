...
void WiFiManager::setupConfigPortal() {
  ...
  server->on(String(F("/update")).c_str(), std::bind(&WiFiManager::handleUpdate, this));
  server->on(String(F("/doupdate")).c_str(), HTTP_POST, std::bind(&WiFiManager::handleDoUpdate, this), std::bind(&WiFiManager::handleDoUpdateUpload, this));
  ...
  server->on(String(F("/r")).c_str(), std::bind(&WiFiManager::handleRestart, this));
...
void WiFiManager::setUpdaterError() {
...
void WiFiManager::handleRoot() {
  ...
  page += F("<p style=\"text-align: center; font-size: smaller;\">");
  page += F("Versi&oacute;n: ");
  page += F("<br/>");
  page += __DATE__;
  page += F(" / ");
  page += __TIME__;
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      page += F("No se han encontrado redes. Refresca para escaner de nuevo.");
    } else {
      page += "<h3>Redes disponibles</h3>";
      //sort networks
      int indices[n];
      for (int i = 0; i < n; i++) {
        indices[i] = i;
...
/** Handle update of the firmare from internet */
void WiFiManager::handleUpdate() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("<form method='post' action='/doupdate' enctype='multipart/form-data'>");
  page += F("<h3>Actualizar firmware</h3>");
  page += F("<p>Selecciona un archivo .bin o .bin.gz con el nuevo firmware:</p>");
  page += F("<input type='file' accept='.bin,.bin.gz' name='firmware'>");
  page += F("<button type='submit'>Actualizar firmware</button>");
  page += F("</form>");
  /*
  page += F("<form method='post' action='' enctype='multipart/form-data'>");
  page += F("FileSystem:<br>");
  page += F("<input type='file' accept='.bin,.bin.gz' name='filesystem'>");
  page += F("<button type='submit'>Update FileSystem</button>");
  page += F("</form>");
  */
  page += F("<p>");
  page += F("Si la ventana de selecci&oacute;n de archivo no se abre, copia el siguiente link y p&eacute;galo en tu navegador: ");
  page += F("<div style=\"color: blue; text-decoration: underline;\"><script>document.write(window.location.href.split('?')[0]);</script></div>");
  page += F("</p>");
  page += FPSTR(HTTP_END);
...
/** handle the /doupdate form POST (once file upload finishes) */
void WiFiManager::handleDoUpdate() {
    DEBUG_WM(F("Do update:"));
    ...
    if (Update.hasError()) {
        ...
        page += FPSTR(HTTP_SUCESS_RESPONSE);
    }
...
void WiFiManager::handleDoUpdateUpload() {
    DEBUG_WM(F("Do update upload:"));
...
void WiFiManager::handleRestart() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("El módulo se reiniciará en breves segundos.");
  page += FPSTR(HTTP_END);
...
void WiFiManager::setSaveConfigCallback( void (*func)(void) ) {
  _savecallback = std::bind(func);
}

//start up save config callback
void WiFiManager::setSaveConfigCallback( std::function<void(void)> func ) {
  _savecallback = func;
}
